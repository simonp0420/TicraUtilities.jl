...
@safetestset "read_cutfiles" begin 
    using TicraUtilities
    t2 = read_cutfiles(joinpath(@__DIR__, "test2.cut"))
    @test length(t2) == 3
...
@safetestset "Cut power" begin 
    ...
    ts = read_cutfiles(joinpath(@__DIR__, "test2.cut"))
    for t1 in ts
...
@safetestset "write_ticra_cut" begin 
    ...
    cuts1 = read_cutfiles(joinpath(@__DIR__, "test2.cut"))
    tfile = joinpath(tempdir(),"temp.cut")
    write_ticra_cut(tfile, cuts1, "Cut file normalized to directivity")
    cuts2 = read_cutfiles(tfile)
    ...
    cuts2_3components = read_cutfiles(tfile)
